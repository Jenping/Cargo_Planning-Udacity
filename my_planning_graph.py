...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1_eff in node_a1.effnodes:
            for a2_eff in node_a2.effnodes:
                if a1_eff.symbol == a2_eff.symbol:
                    if a1_eff.is_pos != a2_eff.is_pos:
                        return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1_eff in node_a1.effnodes:
            for a2_pre in node_a2.prenodes:
                if a1_eff.symbol == a2_pre.symbol:
                    if a1_eff.is_pos != a2_pre.is_pos:
                        return True
                    
        for a1_pre in node_a1.prenodes:
            for a2_eff in node_a2.effnodes:
                if a1_pre.symbol == a2_eff.symbol:
                    if a1_pre.is_pos != a2_eff.is_pos:
                        return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for s_level in self.s_levels:
            ...
            for s_node in s_level:
                if len(goals) == 0:
                    break
                if s_node.is_pos and s_node.symbol in goals:
                    level_sum += level
                    goals.remove(s_node.symbol)
            level += 1
        return level_sum
